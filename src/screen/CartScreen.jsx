...
const CartScreen = () => {
  const { cartItems, deleteCartItem, clearCart, updateCartItemQuantity } = useContext(CartContext);
  const navigation = useNavigation();
  ...
  const handleDecreaseQuantity = async (id, quantity) => {
    if (quantity > 1) {
      ...
      await handleDeleteItem(id); // Remove item if quantity goes to zero
    }
  };
  ...
  const handleRazorpayPayment = async () => {
    try {
      const response = await fetch("https://api.marasimpex.com/api/orders/create-order", {
        ...
        body: JSON.stringify({ amount: totalPrice }), // Sending the total price
      });
    ...
    try {
      const response = await fetch("https://api.marasimpex.com/api/orders/checkout", {
        ...
        body: JSON.stringify({
          ...
          totalPrice, // No GST, just the total price
        }),
      });
      ...
      <View style={styles.container}>
        {cartItems.length === 0 ? (
          <View style={styles.emptyCartContainer}>
            <Text style={styles.emptyCartText}>Oops! Your Cart is Empty</Text>
            <TouchableOpacity style={styles.cartbutton} onPress={() => navigation.navigate("HOME")}>
              <Text style={styles.cartButtonText}>Go to Shop</Text>
            </TouchableOpacity>
          </View>
        ) : (
          <>
